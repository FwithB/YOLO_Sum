...
def deploy_model_to_nuclio(model_path=None, function_name=None,
    ...
    if model_path is None or not os.path.exists(model_path):
        ...
        # 添加这一段
        if model_path is None:
            print("[DEBUG] Using get_latest_model_from_runs")
...
if __name__ == '__main__':
    ...
    parser.add_argument("model_path", nargs="?", help="模型文件路径")
    parser.add_argument("--name", help="函数名")
    ...
    if model_path and ':\\' in model_path:
        model_path = convert_windows_path_to_wsl(model_path)
        print(f"[DEBUG] Converted Windows path to WSL: {model_path}")
    
    deploy_res = deploy_model_to_nuclio(
