...
# 添加新函数在这里
def open_annotation_interface_sync(task_id, job_id=1):
    """同步版本，保持浏览器实例不被销毁"""
    import threading
    import time
    ...
    def run_browser():
        ...
        try:
            ...
            local_controller = BrowserController()
            
            async def open_and_keep():
                await local_controller.initialize(browser_type="msedge", headless=False)
                await local_controller.navigate("http://localhost:8080/auth/login")
                await local_controller.login_cvat()
                url = f"http://localhost:8080/tasks/{task_id}/jobs/{job_id}"
                await local_controller.navigate(url)
                await local_controller.page.wait_for_selector('.cvat-canvas-container', timeout=20000)
                return {"status": "success", "message": f"Opened annotation interface"}
            
            result.update(loop.run_until_complete(open_and_keep()))
